...
class FileList():
    ...
    def compare(self, other_list):
        ...
        @param other_list: A Dictionary of files from a FileList (specifcally
                           FileList.file_list)
        """
        ...
        for file in self.file_list:
            try:
                if self.md5 is True:
                    ...
                    _log.debug('HASH Local: %s Remote %s' % (
                        self.file_list[file]['hash'],
                        other_list[file]['hash']))
                    if other_list[file]['hash'] == \
                            self.file_list[file]['hash']:
                ...
                if other_list[file]['size'] == self.file_list[file]['size']:
                    ...
                    self.add(file)
                if other_list[file]['last_modified'] >= \
                        self.file_list[file]['last_modified']:
                    _log.debug('last_modified is newer - Ignoring')
                else:
                    _log.debug('last_modified is Older - adding to sync list')
                    self.add(file)
            except KeyError:
                _log.info('File %s not found in other file list - Adding to '
                          'sync list' % file)
                self.add(file)
